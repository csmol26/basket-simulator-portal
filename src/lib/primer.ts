...
export const initPrimer = async (config: PrimerCheckoutConfig): Promise<void> => {
  ...
  <div slot="card-form-content" style="--primer-input-height: 40px; --primer-space-medium: 16px; display: flex; flex-direction: column; gap: 16px;">
    ...
    <primer-input-card-number -input-card-number>label="Card Number" placeholder="**** **** **** ****" aria-label="Card Number" ></primer
  </div>
    <div style="flex: 1; margin-bottom: 8px;">
    <primer-input-card-expiry -input-card-expiry>label="Expiry Date" placeholder="**/**" aria-label="Card Expiry Date" ></primer
  </div>
    <div style="flex: 1; margin-bottom: 8px;">
    <primer-input-cvv -input-cvv>label="CVV" placeholder="***" aria-label="Card Security Code" ></primer
  </div>
</div>
<primer-input-card-holder-name -input-card-holder-name>label="Cardholder Name" placeholder="John Smith" aria-label="Cardholder Name" ></primer
